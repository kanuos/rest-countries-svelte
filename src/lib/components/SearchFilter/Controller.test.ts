...
describe(Controller.name, () => {
	...
	it('should render the region selector', async () => {
		const regionTogglerBtn = getByRole('button', { name: 'region menu toggler' });
		...
		regionItems?.forEach((btn, i) => {
			if (i !== 0) expect(btn.textContent).toMatch(`Region ${i}`);
		});
		...
		const updatedBtn = queryByRole('button', { name: 'region menu toggler' });
