...
export const load: PageServerLoad = async ({ params }) => {
	...
	const response = await fetch(
		`${BASE_URL}/alpha/${country}?fields=name,population,region,subregion,capital,tld,currencies,languages,borders,flags`,
		{
			method: 'GET',
			headers: {
	...
	if (countryDetail.borders) {
		const borderURLs = countryDetail.borders?.map(
			(code) => `${BASE_URL}/alpha/${code}?fields=name,cca3`
		);
		...
		countryDetail.borders = allBorderInfo
			.filter((el) => !!el)
			.map((el) => ({ name: el.name.common, cca3: el.cca3 }));
	}
